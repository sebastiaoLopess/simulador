...
def consulta_vendas_simulador(modelo):
    url = 'http://200.194.101.205:8000/consulta_vendas_simulacao'
    response = requests.get(url, params={'familia': modelo})
    try:
        data = response.json()
...
def consulta_estoque_vu(modelo):
    url = 'http://200.194.101.205:8000/estoque'
    response = requests.get(url, params={'modelo': modelo})
    try:
        data = response.json()
